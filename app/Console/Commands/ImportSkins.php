...
class ImportSkins extends Command
{
    ...
    {
        $this->startPage = $this->ask("What is the minimum page number?");
        $this->maxPage = $this->ask("What is the max page number?");
        $sortation = $this->confirm("Do you want to sort by trending?");
        if($sortation) {
            $this->browseSort = "trend";
        }
        $this->getIndexURLs();
        $this->getItemURLs();
        $this->getItemInfo();
    ...
    private function getIndexURLs()
    {
        ...
        for ($i = $this->startPage; $i <= $this->maxPage; $i++) {
            array_push($this->indexURLs, "https://steamcommunity.com/workshop/browse/?appid=252490&browsesort={$this->browseSort}&section=mtxitems&actualsort=mostrecent&p={$i}");
        }
